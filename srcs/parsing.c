...
/*   Updated: 2018/05/26 16:12:39 by vmiachko         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		room(t_union *un, char *str)
{
	...
	while (room)
	{
		if (!ft_strcmp(room->name, arr[0]) ||
			(ft_atoi(arr[1]) == room->position.x &&
				ft_atoi(arr[2]) == room->position.y))
			return (0);
		room = room->next;
	}
	if (!un->room)
...
int				check_input(t_union *un, char *str)
{
	if (is_number(str) && !un->flag.ants)
	{
		...
		return (room(un, str));
	else if (un->flag.ants && un->flag.start == -1
			&& un->flag.end == -1 && is_link(un, str))
	{
		un->flag.links = 1;
		return (1);
	}
